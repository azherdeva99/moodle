 ...
 */
class moodle1_mod_random_handler extends moodle1_mod_handler {
    ...
    public function get_paths() {
        return array(
            new convert_path(
                'random', '/MOODLE_BACKUP/COURSE/MODULES/MOD/RANDOM',
                array(
                    'renamefields' => array(
                        'description' => 'intro',
                        'format' => 'introformat',
     ...
     * data available
     */
    public function process_random($data) {
        global $CFG;
        ...
        $this->currentsubpluginname = $data['randomtype'];

        // get a fresh new file manager for this instance
        $this->fileman = $this->converter->get_file_manager($contextid, 'mod_random');
        ...
        $this->open_xml_writer("activities/random_{$this->moduleid}/random.xml");
        $this->xmlwriter->begin_tag('activity', array('id' => $instanceid, 'moduleid' => $this->moduleid,
            'modulename' => 'random', 'contextid' => $contextid));
        $this->xmlwriter->begin_tag('random', array('id' => $instanceid));
        ...
        $this->handle_random_subplugin($data);
     ...
     * data available
     */
    public function process_random_submission($data) {
        //@todo process user data
     ...
     */
    public function handle_random_subplugin($data) {
        $handler = $this->get_subplugin_handler($this->currentsubpluginname);
        $this->log('Instantiated random subplugin handler for '.$this->currentsubpluginname.'.', backup::LOG_DEBUG);
        $handler->use_xml_writer($this->xmlwriter);

        $this->log('Processing random subplugin handler callback for '.$this->currentsubpluginname.'.', backup::LOG_DEBUG);
     ...
     */
    public function on_random_end() {
        // finish writing assignment.xml
        $this->xmlwriter->end_tag('submissions');
        $this->xmlwriter->end_tag('random');
        $this->xmlwriter->end_tag('activity');
        $this->close_xml_writer();

        // write inforef.xml
        $this->open_xml_writer("activities/random_{$this->moduleid}/inforef.xml");
    ...
    protected function get_subplugin_handler($subplugin) {
        ...
        if (is_null($this->subpluginhandlers)) {
            ...
            $subplugins = core_component::get_plugin_list('random');
            foreach ($subplugins as $name => $dir) {
                $handlerfile  = $dir.'/backup/moodle1/lib.php';
                $handlerclass = "moodle1_mod_random_{$name}_subplugin_handler";
                ...
                $this->log('preparing random subplugin handler', backup::LOG_DEBUG, $handlerclass);
                $this->subpluginhandlers[$name] = new $handlerclass($this, $name);
                if (!$this->subpluginhandlers[$name] instanceof moodle1_random_subplugin_handler) {
        ...
        if (!isset($this->subpluginhandlers[$subplugin])) {
            ...
            $this->subpluginhandlers[$subplugin] = new moodle1_random_unsupported_subplugin_handler($this, $subplugin);
        }
 ...
 */
abstract class moodle1_random_subplugin_handler extends moodle1_submod_handler {
     ...
     */
    public function __construct(moodle1_mod_handler $randomhandler, $subpluginname) {
        parent::__construct($randomhandler, 'random', $subpluginname);
    }
 ...
 */
class moodle1_random_unsupported_subplugin_handler extends moodle1_random_subplugin_handler {
}
