 ...
 */
class backup_random_activity_structure_step extends backup_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        $random = new backup_nested_element('random', array('id'), array(
            'name', 'intro', 'introformat', 'randomtype',
        ...
        $this->add_subplugin_structure('random', $random, false);

        $random->add_child($submissions);
        $submissions->add_child($submission);

        // Apply for 'assignment' subplugins optional stuff at submission level (not multiple)
        $this->add_subplugin_structure('random', $submission, false);

        // Define sources
        $random->set_source_table('random', array('id' => backup::VAR_ACTIVITYID));

        // All the rest of elements only happen if we are including user info
        if ($userinfo) {
            $submission->set_source_table('random_submissions', array('random' => backup::VAR_PARENTID));
        }

        // Define id annotations
        $random->annotate_ids('scale', 'grade');
        $submission->annotate_ids('user', 'userid');
        $submission->annotate_ids('user', 'teacher');

        // Define file annotations
        $random->annotate_files('mod_random', 'intro', null); // This file area hasn't itemid
        $submission->annotate_files('mod_random', 'submission', 'id');
        $submission->annotate_files('mod_random', 'response', 'id');

        // Return the root element (assignment), wrapped into standard activity structure
        return $this->prepare_activity_structure($random);
    }
}
